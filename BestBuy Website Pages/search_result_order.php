...
if(isset($_SESSION["cart_products"]) && count($_SESSION["cart_products"])>0)
{
	...
	foreach ($_SESSION["cart_products"] as $cart_itm)
	{
		...
		echo '<td>Qty <input type="text" size="2" maxlength="2" name="product_qty['.$productid.']" value="'.$product_qty.'" style="width:40px;"/></td>';
		echo '<td>'.$gwpname.'</td>';
...
<h1 class="w3-text-teal" align="left" style="margin:2px"><?php $ctg = $_SESSION['ctg']; echo($ctg).' - '.$_SESSION['sname'] ?></h1>
</div>
...
//    print "<script type='text/javascript'>alert('{$_GET['ord']}');location='search_result.php'; </script>";
    
    
// whether ordered by price
if($_GET['ord']==1)
    $results = $mysqli->query("SELECT p.productid, p.gwpname, p.cost, p.description, s.amount,  t.storeName, c.gwcname AS category, b.gwbname AS brand FROM product P, category C, brand B, storage S, store T where S.storeID = T.storeID and T.storeName = '{$_SESSION['sname']}' and P.categoryID = C.categoryID AND B.brandID = P.brandID and s.productID = p.productid and c.gwcname='{$ctg}' and s.amount > 0  ORDER BY cost DESC");  
else
    $results = $mysqli->query("SELECT p.productid, p.gwpname, p.cost, p.description, s.amount,  t.storeName, c.gwcname AS category, b.gwbname AS brand FROM product P, category C, brand B, storage S, store T where S.storeID = T.storeID and T.storeName = '{$_SESSION['sname']}' and P.categoryID = C.categoryID AND B.brandID = P.brandID and s.productID = p.productid and c.gwcname='{$ctg}' and s.amount > 0  ORDER BY cost ASC"); 
